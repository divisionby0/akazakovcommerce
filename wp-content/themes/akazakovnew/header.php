 ...
 * @since Camp 1.0
 */
if($_SERVER['HTTP_HOST'] == "alexandrkazakov.ru"){
...
$main_theme = get_template_directory_uri();
$webcoupeData = new WebcoupeData;
...
<!--<script defer type="text/javascript" src="//vk.com/js/api/openapi.js?49"></script> -->
...
?>
<style>
.page_vskryt-korobku .top-menu-head {
    opacity: 0.3;
}
.page_vskryt-korobku .top-menu-head:hover {
    opacity: 1;
}
</style>
<?
...
<!-- End Google Tag Manager -->
    ...
    <!--TODO edited by divby0 https://freelancehunt.com/freelancer/divisioby0.html -->
    <style>
        html, body{
        ...
        .top-menu-head{
            ...
            $isAtLessonPage = strpos($actual_link, '/lessons/') !== false;

            if ($isAtLessonPage == true) {
                echo "margin-top: 40px!important;";
            }
            ?>
        ...
        <header>
            <div class = "top-menu-head">
              ...
              <?php
                    ...
                    $isBioPage = $post_slug === "moja-istoriya";
                    $pageCourseIDs = get_option("getCoursePageIDs");
              
                    $coursePageSlugs = explode(",", $pageCourseIDs);
              
                    $isCoursePage = in_array($post_slug, $coursePageSlugs);
              
         			if(is_attachment())
                    {
         				include "headers/head_simple.php";
         			}
                     else if(is_home()){
                         include "headers/head_main.php";
                     } 
                     elseif(is_single() || is_category() || is_page(52))
                     {
                         if($isCoursePage == 1 || $isCoursePage == true){
                             // TODO do not include any header
                         }
                         else{
                             include "headers/head_single.php";
                         }
                     }
                     elseif($isBioPage === true){
                         include "headers/head_story.php";
                     }
                     elseif(is_page(49)){
                         //echo "<p>adding head proj</p>";
                         include "headers/head_proj.php";
                     }
              ?>
                     <div class="clear"></div>
		</header>
