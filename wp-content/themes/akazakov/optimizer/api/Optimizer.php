...
class Optimizer{
	...
	public $excludeScriptsSrc = array("admin-bar.min.js", "150828645374452.js", "jquery.min.js");
...
//					$dom->load($dom->save());
		...
		foreach($dom->find('link[rel="stylesheet"]') as $style){
			//echo "style: <pre>".var_export($style->href, true)."</pre>";
			if(strlen($style->href) > 0){
				...
				//echo "domain: <pre>".var_export($domain, true)."</pre>";
				//echo "check_local_url: <pre>".var_export($this->check_local_url($style->href), true)."</pre>";
				//if(strpos($style->href, $domain) !== false or $isLocalUrlByProtocol = $this->check_local_url($style->href)){
					//if(function_exists("plugins_url") and $isLocalUrlByProtocol){
				if(strpos($style->href, $domain) !== false){
					//echo "style local: <pre>".var_export($style->href, true)."</pre>";
					if(function_exists("plugins_url")){
						$src_array = explode(str_replace(get_site_url(), "", plugins_url()), $style->href);
						array_shift($src_array);
						//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
						//$style->href = plugins_url().implode("", $src_array); //removed what is it??
					}
					//echo "style local1: <pre>".var_export($style->href, true)."</pre>";
					$resources['styles']['local'][] = $style;
					$style->outertext = '';
//					$dom->load($dom->save());
				}
				else{
					//echo "style remote: <pre>".var_export($style->href, true)."</pre>";
...
//				}
	...
	public function convertImagesToBase64($image){	
		...
		if(!in_array($imageFileName, $this->excludeImagesFromBase64Converting)){
			...
			if(strpos($image, $domain) !== false){
				...
				if(function_exists("wp_upload_dir") and strpos($image, $upload_dir) !== false ){
			...
			if($iExt and $fileContent){
				//echo "image: ".var_export(array('base64Code' => "data:image/".$iExt.";base64,".base64_encode($fileContent),	'imageSelector' => $imageSelector), true);
				return array(
					'base64Code' => "data:image/".$iExt.";base64,".base64_encode($fileContent),
	...
	public function url_exists($url){
	   ...
	   //echo "headers: <pre>".var_export($headers,true)."</pre>";
