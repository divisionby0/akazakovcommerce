	...
	class LP_Abstract_User extends LP_Abstract_Object_Data {
		...
		public function can_view_item( $item_id = 0, $course_id = 0 ) {
			...
			if ( $course && $course->is_publish() && $item ) {
				if($item instanceof LP_Lesson || $item instanceof LP_Quiz){
					if($checkUserAvailabilityResult["result"] == true || $checkUserAvailabilityResult["result"] == 1){
						$view = 'no-required-enroll';
					}
					else{
						if($checkUserAvailabilityResult["error"] == AppError::$NOT_LOGGED_IN){
							$view = AppError::$NOT_LOGGED_IN;
						}
						else{
							$view = AppError::$NOT_PAYED;
						}
					}
				}
				else if($item instanceof LP_Course){
					$view = 'no-required-enroll';
				}
